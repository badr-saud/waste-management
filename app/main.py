        ...
        try:
            ...
            if isinstance(data, dict) and "values" in data and isinstance(data["values"], list):
                ...
                if isinstance(data, dict):
                    ...
                    values = [float(val) for val in str(data).replace("[", "").replace("]", "").split(",")[:3]]
        except Exception as json_error:
            logger.error(f"JSON parsing error: {json_error}")
            # If JSON parsing fails, try to extract numbers from the string
            try:
                import re
                values = [float(val) for val in re.findall(r"[-+]?\d*\.\d+|\d+", body.decode())[:3]]
            except Exception as regex_error:
                logger.error(f"Regex extraction error: {regex_error}")
                raise HTTPException(status_code=400, detail="Could not parse input data")
